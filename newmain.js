...
var reduceLeft = 0;
var addscoreList = [];
var deadSound = new Audio('explosion.mp3'); // 死亡したときの効果音ファイルを読み込む
var invincibleSound = new Audio('invincible.mp3');
var deleteSound = new Audio('delete.mp3');
var reduceSound = new Audio('reduce.mp3');
...
window.onload = function(){
  ...
  player.speed = reSet(player.speed,basic,window.innerHeight);
  enemy.size = reSet(enemy.size,basic,window.innerHeight);
  player.bulletsize = reSet(player.bulletsize,basic,window.innerHeight);
  Bullet.speed = reSet(Bullet.speed,basic,window.innerHeight);
  enemy.bulletsize = reSet(enemy.bulletsize,basic,window.innerHeight);
  EnemyBullet.speed = reSet(EnemyBullet.speed,basic,window.innerHeight);
  Item.speed = reSet(Item.speed,basic,window.innerHeight);
  itemsize = reSet(itemsize,basic,window.innerHeight);
        ...
        for (var i = deleteList.length-1; i >= 0 ; i--) {
          ...
          function resizeCanvas() {
            ...
            for (var i = enemyBulletList.length-1; i >= 0 ; i--) {
              ...
              enemyBulletList[i].x = reSet(enemyBulletList[i].x,basic,window.innerHeight);
              enemyBulletList[i].y = reSet(enemyBulletList[i].y,basic,window.innerHeight);
              enemyBulletList[i].speed = reSet(enemyBulletList[i].speed,basic,window.innerHeight);
            }
            enemy.bulletsize = reSet(enemy.bulletsize,basic,window.innerHeight);
            EnemyBullet.speed = reSet(EnemyBullet.speed,basic,window.innerHeight);
            for (var i = bulletList.length-1; i >= 0 ; i--) {
              bulletList[i].size = reSet(player.bulletsize,basic,window.innerHeight);
              bulletList[i].x = reSet(bulletList[i].x,basic,window.innerHeight);
              bulletList[i].y = reSet(bulletList[i].y,basic,window.innerHeight);
              bulletList[i].speed = reSet(bulletList[i].speed,basic,window.innerHeight);
            }
            player.bulletsize = reSet(player.bulletsize,basic,window.innerHeight);
            Bullet.speed = reSet(Bullet.speed,basic,window.innerHeight);
            for (var i = deleteList.length-1; i >= 0 ; i--) {
              deleteList[i].size = reSet(itemsize,basic,window.innerHeight);
              deleteList[i].x = reSet(deleteList[i].x,basic,window.innerHeight);
              deleteList[i].y = reSet(deleteList[i].y,basic,window.innerHeight);
              deleteList[i].speed = reSet(deleteList[i].speed,basic,window.innerHeight);
            }
            for (var i = reduceList.length-1; i >= 0 ; i--) {
              reduceList[i].size = reSet(itemsize,basic,window.innerHeight);
              reduceList[i].x = reSet(reduceList[i].x,basic,window.innerHeight);
              reduceList[i].y = reSet(reduceList[i].y,basic,window.innerHeight);
              reduceList[i].speed = reSet(reduceList[i].speed,basic,window.innerHeight);
            }
            for (var i = addscoreList.length-1; i >= 0 ; i--) {
              addscoreList[i].size = reSet(itemsize,basic,window.innerHeight);
              addscoreList[i].x = reSet(addscoreList[i].x,basic,window.innerHeight);
              addscoreList[i].y = reSet(addscoreList[i].y,basic,window.innerHeight);
              addscoreList[i].speed = reSet(addscoreList[i].speed,basic,window.innerHeight);
            }
            for (var i = invincibleList.length-1; i >= 0 ; i--) {
              invincibleList[i].size = reSet(itemsize,basic,window.innerHeight);
              invincibleList[i].x = reSet(invincibleList[i].x,basic,window.innerHeight);
              invincibleList[i].y = reSet(invincibleList[i].y,basic,window.innerHeight);
              invincibleList[i].speed = reSet(invincibleList[i].speed,basic,window.innerHeight);
            }
            itemsize = reSet(itemsize,basic,window.innerHeight);
            basic = window.innerHeight;
          }
